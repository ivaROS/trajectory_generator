...
void TrajectoryGeneratorBridge::generate_trajectory(const nav_msgs::Odometry& curr_odom, traj_func* trajpntr)
{
    state_type x0(8);
...
void TrajectoryGeneratorBridge::initFromOdom(const nav_msgs::Odometry& curr_odom, state_type& x0)
{
...
std::vector<turtlebot_trajectory_controller::trajectory_point> TrajectoryGeneratorBridge::TrajectoryFromStates(std::vector<state_type> x_vec, std::vector<double> times)
{
    std::vector<turtlebot_trajectory_controller::trajectory_point> trajectory;
    for(int i = 0; i < x_vec.size(); i++)
    {
        turtlebot_trajectory_controller::trajectory_point point;
...
const nav_msgs::Odometry TrajectoryGeneratorBridge::OdomFromState(state_type& state, double t, std_msgs::Header header)
{
